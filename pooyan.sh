  ...
  case "${LANG_CODE}" in
    fa)
      case "$key" in
        ...
        quick_desc_1) echo "حالت سریع نیازی به دامنه Cloudflare ندارد و با CF Argo Quick Tunnel لینک سریع می‌سازد." ;;
        quick_desc_2) echo "حالت سریع بعد از ریبوت یا اجرای دوباره اسکریپت از کار می‌افتد و باید دوباره ساخته شود." ;;
        install_desc_1) echo "حالت نصب سرویس نیاز به دامنه متصل به Cloudflare دارد و باید Argo را دستی طبق راهنما متصل کنید." ;;
        install_desc_2) echo "بعد از اولین اتصال Argo، با نگه‌داشتن پوشه /root/.cloudflared می‌توانید ورود دوباره را رد کنید." ;;
        site_light) echo "Pooyan Light:" ;;
        site_nat) echo "Pooyan NAT:" ;;
        reboot_note) echo "توجه: حالت سریع بعد از ریبوت سرور غیرفعال می‌شود." ;;
        slogan) echo "Pooyan 0.05" ;;
        ...
        menu_0) echo "0. خروج" ;;
        choose_mode) echo "حالت را انتخاب کنید (پیش‌فرض 1): " ;;
        choose_protocol) echo "پروتکل xray را انتخاب کنید (پیش‌فرض 1=vmess, 2=vless): " ;;
        choose_ip_mode) echo "حالت اتصال argo را انتخاب کنید IPV4 یا IPV6 (4 یا 6، پیش‌فرض 4): " ;;
        ...
        github_title) echo "GitHub و نصب" ;;
        github_repo) echo "مخزن:" ;;
        github_branch) echo "شاخه:" ;;
        github_install) echo "نصب یک‌خطی GitHub:" ;;
        github_note) echo "این دستور وقتی کار می‌کند که همین فایل را با نام pooyan.sh در ریشه repo قرار بدهی." ;;
